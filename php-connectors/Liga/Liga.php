...
define("__CLASSNAME__", "\\Liga");

use com\anotherservice\util as cau;
use com\anotherservice\db as cad;

class Liga extends com\busit\Connector implements com\busit\Producer, com\busit\Consumer
{
	public function produce($out)
	{
		...
		$url = 'http://www.matchendirect.fr/rss/foot-primera-division-c14.xml';
			...
			if( count($news) > 0 )
			{
				...
				{	
					...
					if( !$row['buffer_identifier'] && ( $this->config('team') == 'All' || $this->config('team') == 'Toutes' || strpos($n['title'], $this->config('team')) !== false ) && strpos($n['title'], 'final' ) !== false )
					{
						...
						$content['date'] =  $n['date'];
						$content['author'] = 'Liga';
						$content['image'] = 'https://images.busit.com/connectors/110_100.png';
