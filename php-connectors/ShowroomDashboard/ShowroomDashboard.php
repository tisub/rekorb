...
define("__CLASSNAME__", "\\ShowroomDashboard");

use com\busit as cb;
use com\anotherservice\util as cau;
use com\anotherservice\db as cad;

class ShowroomDashboard implements cb\IConnector
{
	private $url = 'http://bi-001.vlan-101:8081/showroomdashboard';
	private $config;
	private $inputs;
	private $outputs;
	private $uid;
	...
	public function setInput($message, $interfaceId)
	{
		...
		else
		{
			if( !$msg['number'] )
			{
				if( is_numeric($msg['message']) )
					$data['number'] = $msg['message'];
				else if( is_numeric($msg->toString()) )
					$data['number'] = $msg->toString();
			}
			else
		...
		if( $data['number'] )
			$data['number'] = round($data['number'], 2);

		$send = array('type'=>$this->inputs[$interfaceId]['key'], 'interface'=>$interfaceId, 'instance'=>$this->instance, 'message'=>$data);
		$send = json_encode($send);
		
		$request = array('data'=>$send);
		
		if( $file['name'] )
			$request['file'] = Unirest::file('/tmp/' . $file['name']);
		
		$response = Unirest::post($this->url, array( "Accept" => "application/json" ), $request);
