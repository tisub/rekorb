...
define("__CLASSNAME__", "\\PremierLeague");

use com\anotherservice\util as cau;
use com\anotherservice\db as cad;

class PremierLeague extends com\busit\Connector implements com\busit\Producer, com\busit\Consumer
{
			...
			if( count($news) > 0 )
			{
				...
				{	
					...
					if( !$row['buffer_identifier'] && ( $this->config('team') == 'All' || $this->config('team') == 'Toutes' || strpos($n['title'], $this->config('team')) !== false ) && strpos($n['title'], 'final' ) !== false )
					{
						...
						$content['date'] =  $n['date'];
						$content['author'] = 'Premier League';
						$content['image'] = 'https://images.busit.com/connectors/112_100.png';
