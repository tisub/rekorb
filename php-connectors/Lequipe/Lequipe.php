...
define("__CLASSNAME__", "\\Lequipe");

use com\anotherservice\util as cau;
use com\anotherservice\db as cad;

class Lequipe extends com\busit\Connector implements com\busit\Producer, com\busit\Consumer
{
	public function produce($out)
	{
		...
		switch( $out->key )
		{
			...
			break;
			case 'basket':
				$url = 'http://www.lequipe.fr/rss/actu_rss_Basket.xml';
			break;
			case 'tennis':
				$url = 'http://www.lequipe.fr/rss/actu_rss_Tennis.xml';
			break;
			case 'rugby':
				$url = 'http://www.lequipe.fr/rss/actu_rss_Rugby.xml';
			break;
			case 'automoto':
				$url = 'http://www.lequipe.fr/rss/actu_rss_Auto-Moto.xml';
			break;
			case 'general':
			...
			if( count($news) > 0 )
			{
				foreach( $news as $n )
				{
					...
					{						
						...
						$content['date'] =  $n['date'];
						$content['author'] = "L'équipe";
						$content['image'] = 'https://images.busit.com/connectors/56_100.png';
