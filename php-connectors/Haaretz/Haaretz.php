...
define("__CLASSNAME__", "\\Haaretz");

use com\anotherservice\util as cau;
use com\anotherservice\db as cad;

class Haaretz extends com\busit\Connector implements com\busit\Producer, com\busit\Consumer
{
	public function produce($out)
	{
		...
		{
			case 'defense':
				$url = 'http://feeds.feedburner.com/DefenseAndDiplomacy?format=xml';
			break;
			case 'international':
				$url = 'http://feeds.feedburner.com/InternationalRss?format=xml';
			break;
			case 'national':
				$url = 'http://feeds.feedburner.com/haaretz/National?format=xml';
			break;
			case 'news':
				$url = 'http://feeds.feedburner.com/haaretz/LBao?format=xml';
			break;
			default:
				$url = 'http://feeds.feedburner.com/haaretz/LBao?format=xml';
		}
			...
			if( count($news) > 0 )
			{
				...
				{	
					...
					if( !$row['buffer_identifier'] )
					{
						...
						$content['date'] =  $n['date'];
						$content['author'] = 'Haaretz';
						$content['image'] = 'https://images.busit.com/connectors/236_100.png';
