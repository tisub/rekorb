...
define("__CLASSNAME__", "\\GoogleTranslate");

use com\busit as cb;
use com\anotherservice\util as cau;

class GoogleTranslate extends com\busit\Connector implements com\busit\Transformer
{
	private $key = 'xxx';
	...
	private function translate($text, $language)
	{
		...
		try
		{
			$detect = file_get_contents("https://www.googleapis.com/language/translate/v2/detect?key={$this->key}&q=".urlencode($text));
			...
			$result = file_get_contents("https://www.googleapis.com/language/translate/v2?key={$this->key}&q=".urlencode($text)."&source={$original_lang}&target={$language}");
